...
public class ListRegisteredAdapter extends RecyclerView.Adapter<ListRegisteredAdapter.ListHolder> {

    private Context context;

    private Map<String, MissingPersonInfo> missingPersonInfos;

    public ListRegisteredAdapter(Context context) {
    ...
    public void onBindViewHolder(ListHolder holder, final int position) {
        ...
        if (isEqual) {
            ...
            holder.like.setOnClickListener(new FullListener(context, holder.like, 0, key));
        } else {
            holder.like.setImageResource(R.drawable.icon_heart_empty);
            holder.like.setOnClickListener(new EmptyListener(context, holder.like, 0, key));
        }
    }
